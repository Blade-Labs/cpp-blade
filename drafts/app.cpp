...
ApiService::AccountData createAccount()
{
  ...
  std::unique_ptr<ECDSAsecp256k1PrivateKey> privateKey =
    ECDSAsecp256k1PrivateKey::fromBIP39Mnemonic(seedPhrase, "")->derive(44)->derive(3030)->derive(0)->derive(0);
  ...
  AccountHelper::executeUpdateAccountTransactions(
    &client, 
    privateKey, 
...
int main(int argc, char** argv)
{
  ...
  std::unique_ptr<ECDSAsecp256k1PrivateKey> ecdsaSecp256k1PrivateKeyMy =
    ECDSAsecp256k1PrivateKey::fromBIP39Mnemonic(mnemonic, "")->derive(44)->derive(3030)->derive(0)->derive(0);
